# Code originally from Onnela Lab, `Beiwe-Analysis` repo; March 2018
...
GPS_preprocessing = function(patient_name,
  ...
  if(file.exists(patient_output_filename)){
    ...
    avg_out_mat = GPS2MobMat(filelist,itrvl=ITRVL,accuracylim=ACCURACY_LIM,r=rad_fp,w=wid_fp)
    mobmatmiss = avg_out_mat$outmat
    ...
    if(qOKmsg!=""){
      ...
      # return(NULL) #ZHE DEBUG
    }
    # ZHE DEBUG (always do this now)
    saveRDS(list(mobmat,mobmatmiss,obj,tz,CENTERRAD,ITRVL),patient_output_filename)
    # zhe added output each interval avg too for clusters
    saveRDS(avg_out_mat$avgmat, paste(patient_output_filepath, "/gps_preprocessed_avgmat.rds",sep=""))
  }
}
...
GPS2MobMat = function(filelist,itrvl=10,accuracylim=51,r=NULL,w=NULL,tint_m=NULL,tint_k=NULL){
  ...
  IDam = 1
  count = 0
  nextline=c(1,tstart+itrvl/2,mat[1,2],mat[1,3]) # (1, midpoint, next_lat, next_long)
  numitrvl=1
  cat("Collapse data within",itrvl,"second intervals...\n")
  for(i in 2:nrow(mat)){
    #ProgressBar(nrow(mat)-1,i-1)
    if(mat[i,1]/1000<tstart+itrvl){ # if next line is within interval
      nextline[3]=nextline[3]+mat[i,2] # update next_lat
      nextline[4]=nextline[4]+mat[i,3] # update next_lon
      numitrvl=numitrvl+1 # average count
    }else{
  ...
  return(list(outmat = outmat, avgmat = avgmat))
}
